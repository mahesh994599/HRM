...
class SalaryDisbursementSchedule(models.Model):
    """
    Model for scheduling automatic salary disbursement.
    On the configured day each month, confirmed payslips are moved to 'paid' status.
    """

    disburse_day = models.CharField(
        max_length=30,
        choices=DAYS,
        default="last day",
        verbose_name=_("Disbursement Day"),
        help_text=_("On this day of every month, confirmed payslips will be auto-disbursed"),
    )
    auto_disburse = models.BooleanField(
        default=False, verbose_name=_("Auto disburse")
    )
    company_id = models.OneToOneField(
        Company,
        on_delete=models.CASCADE,
    ...
    def clean(self):
        if (
            not self.company_id
            and SalaryDisbursementSchedule.objects.filter(company_id=None).exists()
        ):
            if not self.id:
                raise ValidationError(
                    {
                        "company_id": "Auto salary disbursement for all company already exists"
                    }
                )
            all_company = SalaryDisbursementSchedule.objects.filter(
                company_id=None
            ).first()
            if all_company.id != self.id:
                raise ValidationError(
                    {
                        "company_id": "Auto salary disbursement for all company already exists"
                    }
    ...
    def save(self, *args, **kwargs):
        from payroll.scheduler import auto_salary_disburse

        if self.auto_disburse:
            auto_salary_disburse()

        super().save(*args, **kwargs)

    def __str__(self) -> str:
        return f"{self.disburse_day} | {self.company_id}"

    class Meta:
        verbose_name = _("Salary Disbursement Schedule")
        verbose_name_plural = _("Salary Disbursement Schedules")


class DisbursementBankAccount(models.Model):
    """
    Company bank account used as the source for salary disbursements.
    """
    ...
    company_id = models.OneToOneField(
        ...
        blank=True,
        verbose_name=_("Company"),
        help_text=_("Leave blank to use as default for all companies"),
    )
    ...
    account_holder_name = models.CharField(
        max_length=150, verbose_name=_("Account Holder Name")
    )
    branch = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Branch")
    )
    ifsc_code = models.CharField(
        max_length=50, null=True, blank=True, verbose_name=_("IFSC / SWIFT Code")
    )
    any_other_code = models.CharField(
        max_length=50, null=True, blank=True, verbose_name=_("Other Bank Code")
    )
    ...
    def __str__(self):
        company = self.company_id if self.company_id else "All Companies"
        return f"{self.bank_name} - {self.account_number} ({company})"

    class Meta:
        verbose_name = _("Disbursement Bank Account")
        verbose_name_plural = _("Disbursement Bank Accounts")
...
class ICICIBankConfig(models.Model):
    """
    ICICI Connected Banking API configuration.
    Stores credentials needed to make fund transfer API calls.
    """
    ...
    encryption_key = models.CharField(
        max_length=255, null=True, blank=True, verbose_name=_("Encryption Key")
    )
    corp_id = models.CharField(
        max_length=50, verbose_name=_("Corporate ID"),
        help_text=_("Your ICICI Corporate Internet Banking ID"),
    )
    debit_account_no = models.CharField(
        max_length=50, verbose_name=_("Debit Account Number"),
        help_text=_("Company account from which salary will be debited"),
    )
    default_transfer_mode = models.CharField(
        max_length=10, choices=TRANSFER_MODE_CHOICES, default="NEFT",
        verbose_name=_("Default Transfer Mode"),
    )
    api_base_url = models.URLField(
        default="https://apibanking.icicibank.com/api/v1",
        verbose_name=_("API Base URL"),
        help_text=_("Use sandbox URL for testing"),
    )
    is_sandbox = models.BooleanField(
        default=True, verbose_name=_("Sandbox Mode"),
        help_text=_("Enable for testing. Disable for live transactions."),
    )
    is_active = models.BooleanField(default=True, verbose_name=_("Is Active"))

    def __str__(self):
        mode = "SANDBOX" if self.is_sandbox else "LIVE"
        company = self.company_id if self.company_id else "All"
        return f"ICICI Config ({company}) [{mode}]"

    class Meta:
        verbose_name = _("ICICI Bank Configuration")
        verbose_name_plural = _("ICICI Bank Configurations")
...
class DisbursementTransaction(models.Model):
    """
    Tracks each individual salary transfer transaction.
    """

    payslip = models.ForeignKey(
        Payslip, on_delete=models.CASCADE, related_name="disbursement_transactions",
    ...
    beneficiary_account = models.CharField(
        max_length=50, verbose_name=_("Beneficiary Account"),
    )
    beneficiary_ifsc = models.CharField(
        max_length=20, null=True, blank=True, verbose_name=_("Beneficiary IFSC"),
    )
    beneficiary_name = models.CharField(
        max_length=150, verbose_name=_("Beneficiary Name"),
    )
    status = models.CharField(
    ...
    transaction_ref = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Transaction Reference"),
    )
    bank_ref = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Bank Reference No."),
    )
    response_message = models.TextField(
        null=True, blank=True, verbose_name=_("Response Message"),
    )
