...
from payroll.forms.component_forms import (
    ...
    DisbursementBankAccountForm,
    ICICIBankConfigForm,
    PayrollSettingsForm,
    PayslipAutoGenerateForm,
    SalaryDisbursementScheduleForm,
)
...
# ===========================Auto salary disbursement================================


@login_required
@permission_required("payroll.view_salarydisbursementschedule")
def salary_disbursement_settings_view(request):
    disbursement_schedules = SalaryDisbursementSchedule.objects.all()
    context = {"disbursement_schedules": disbursement_schedules}
    return render(
        request, "payroll/settings/salary_disbursement_settings.html", context
    )


@login_required
@hx_request_required
@permission_required("payroll.change_salarydisbursementschedule")
def create_or_update_salary_disbursement(request, schedule_id=None):
    schedule = None
    if schedule_id:
        schedule = SalaryDisbursementSchedule.objects.get(id=schedule_id)
    form = SalaryDisbursementScheduleForm(instance=schedule)
    if request.method == "POST":
        form = SalaryDisbursementScheduleForm(request.POST, instance=schedule)
        if form.is_valid():
            schedule = form.save()
            company = schedule.company_id if schedule.company_id else "All company"
            messages.success(
                request,
                _(f"Salary disbursement for {company} created successfully"),
            )
            return HorillaRedirect(request)
    return render(
        request,
        "payroll/settings/salary_disbursement_create_or_update.html",
        {"form": form},
    )
...
@login_required
@permission_required("payroll.change_salarydisbursementschedule")
def activate_salary_disbursement(request):
    isChecked = request.POST.get("isChecked")
    scheduleId = request.POST.get("scheduleId")
    schedule = SalaryDisbursementSchedule.objects.get(id=scheduleId)
    if isChecked == "true":
        schedule.auto_disburse = True
        response = {
            "type": "success",
            "message": _("Auto salary disbursement activated successfully."),
        }
    else:
        schedule.auto_disburse = False
        response = {
            "type": "success",
            "message": _("Auto salary disbursement deactivated successfully."),
        }
    schedule.save()
    return JsonResponse(response)
...
@login_required
@hx_request_required
@permission_required("payroll.delete_salarydisbursementschedule")
def delete_salary_disbursement(request, schedule_id):
    try:
        schedule = SalaryDisbursementSchedule.objects.get(id=schedule_id)
        if not schedule.auto_disburse:
            company = schedule.company_id if schedule.company_id else "All company"
            schedule.delete()
            messages.success(
                request,
                _(f"Salary disbursement for {company} deleted successfully."),
            )
        else:
            messages.info(
                request, _("Active salary disbursement schedule cannot be deleted.")
            )
        return HorillaRedirect(request)
    except SalaryDisbursementSchedule.DoesNotExist:
        messages.error(request, _("Salary disbursement schedule not found."))
    return HorillaRedirect(request)


# ===========================Salary Disbursement Dashboard================================


@login_required
@permission_required("payroll.view_payslip")
def salary_disbursement_view(request):
    """Main disbursement page with ICICI bank transfer integration."""
    from employee.models import EmployeeBankDetails

    confirmed_qs = Payslip.objects.filter(status="confirmed").select_related("employee_id")
    paid_qs = Payslip.objects.filter(status="paid").select_related("employee_id").order_by("-end_date")[:20]

    def enrich(payslips):
        items = []
        for ps in payslips:
            try:
                bank = EmployeeBankDetails.objects.get(employee_id=ps.employee_id)
            except EmployeeBankDetails.DoesNotExist:
                bank = None
            # Get latest transaction for this payslip
            txn = DisbursementTransaction.objects.filter(payslip=ps).order_by("-initiated_at").first()
            items.append({
                "payslip": ps,
                "bank": bank,
                "has_bank": bank is not None and bank.account_number,
                "txn": txn,
            })
        return items

    confirmed_payslips = enrich(confirmed_qs)
    paid_payslips = enrich(paid_qs)

    no_bank_count = sum(1 for item in confirmed_payslips if not item["has_bank"])

    disbursement_accounts = DisbursementBankAccount.objects.filter(is_active=True)
    icici_config = ICICIBankConfig.objects.filter(is_active=True).first()
    recent_transactions = DisbursementTransaction.objects.all()[:30]

    context = {
        "confirmed_payslips": confirmed_payslips,
        "paid_payslips": paid_payslips,
        "confirmed_count": confirmed_qs.count(),
        "confirmed_total": sum(ps.net_pay or 0 for ps in confirmed_qs),
        "paid_count": paid_qs.count(),
        "paid_total": sum(ps.net_pay or 0 for ps in paid_qs),
        "draft_count": Payslip.objects.filter(status="draft").count(),
        "no_bank_count": no_bank_count,
        "disbursement_accounts": disbursement_accounts,
        "icici_config": icici_config,
        "recent_transactions": recent_transactions,
    }
    return render(request, "payroll/disbursement/salary_disbursement.html", context)


@login_required
@permission_required("payroll.change_payslip")
def disburse_salary(request, payslip_id):
    """Transfer salary to a single employee via ICICI API."""
    if request.method == "POST":
        from decimal import Decimal

        from employee.models import EmployeeBankDetails

        from payroll.icici_service import ICICIBankService

        config = ICICIBankConfig.objects.filter(is_active=True).first()
        if not config:
            messages.error(request, _("ICICI Bank is not configured. Go to Settings to set it up."))
            return redirect("salary-disbursement-view")

        try:
            payslip = Payslip.objects.get(id=payslip_id, status="confirmed")
        except Payslip.DoesNotExist:
            messages.error(request, _("Payslip not found or not in confirmed status."))
            return redirect("salary-disbursement-view")

        try:
            bank = EmployeeBankDetails.objects.get(employee_id=payslip.employee_id)
        except EmployeeBankDetails.DoesNotExist:
            messages.error(request, _(f"No bank details for {payslip.employee_id}."))
            return redirect("salary-disbursement-view")

        emp_name = f"{payslip.employee_id.employee_first_name} {payslip.employee_id.employee_last_name}"
        service = ICICIBankService(config)
        result = service.fund_transfer(
            amount=payslip.net_pay,
            beneficiary_account=bank.account_number,
            beneficiary_ifsc=bank.any_other_code1 or "",
            beneficiary_name=emp_name,
            remarks=f"Salary {payslip.start_date.strftime('%b %Y')}",
        )

        txn = DisbursementTransaction.objects.create(
            payslip=payslip,
            employee=payslip.employee_id,
            amount=Decimal(str(payslip.net_pay)),
            transfer_mode=config.default_transfer_mode,
            beneficiary_account=bank.account_number,
            beneficiary_ifsc=bank.any_other_code1 or "",
            beneficiary_name=emp_name,
            status="success" if result["success"] else "failed",
            transaction_ref=result["txn_ref"],
            bank_ref=result.get("bank_ref", ""),
        ...
        if result["success"]:
            payslip.status = "paid"
            payslip.save()
            messages.success(request, _(f"Transferred {payslip.net_pay} to {emp_name}. Ref: {result['txn_ref']}"))
        else:
            messages.error(request, _(f"Transfer failed for {emp_name}: {result['message']}"))
...
@login_required
@permission_required("payroll.change_payslip")
def disburse_salary_bulk(request):
    """Transfer salary to multiple employees via ICICI API."""
    if request.method == "POST":
        from decimal import Decimal
        ...
        ids_str = request.POST.get("payslip_ids", "")
        if not ids_str:
            messages.warning(request, _("No payslips selected."))
            return redirect("salary-disbursement-view")

        ids = [int(x) for x in ids_str.split(",") if x.strip().isdigit()]
        payslips = Payslip.objects.filter(id__in=ids, status="confirmed").select_related("employee_id")
        ...
        if success_count:
            messages.success(request, _(f"{success_count} salary transfer(s) completed successfully."))
        if fail_count:
            messages.error(request, _(f"{fail_count} transfer(s) failed. Check transaction history."))
...
@login_required
@permission_required("payroll.change_payslip")
def retry_transaction(request, txn_id):
    ...
    if request.method == "POST":
        ...
        config = ICICIBankConfig.objects.filter(is_active=True).first()
        if not config:
            messages.error(request, _("ICICI Bank is not configured."))
            return redirect("salary-disbursement-view")

        try:
            txn = DisbursementTransaction.objects.get(id=txn_id, status="failed")
        except DisbursementTransaction.DoesNotExist:
            messages.error(request, _("Transaction not found or not in failed status."))
            return redirect("salary-disbursement-view")
        ...
        result = service.fund_transfer(
            ...
            beneficiary_ifsc=txn.beneficiary_ifsc or "",
            beneficiary_name=txn.beneficiary_name,
            remarks=f"Retry: Salary",
        )

        txn.retry_count += 1
        txn.transaction_ref = result["txn_ref"]
        txn.bank_ref = result.get("bank_ref", "")
        txn.response_message = result["message"]

        if result["success"]:
            txn.status = "success"
            txn.completed_at = timezone.now()
            txn.payslip.status = "paid"
            txn.payslip.save()
            messages.success(request, _(f"Retry successful for {txn.beneficiary_name}. Ref: {result['txn_ref']}"))
        else:
            txn.status = "failed"
...
@login_required
@hx_request_required
@permission_required("payroll.add_aborticibankconfig")
def create_or_update_icici_config(request, config_id=None):
    config = None
    if config_id:
        config = ICICIBankConfig.objects.get(id=config_id)
    form = ICICIBankConfigForm(instance=config)
    if request.method == "POST":
        form = ICICIBankConfigForm(request.POST, instance=config)
        if form.is_valid():
            form.save()
            messages.success(request, _("ICICI Bank configuration saved."))
            return HorillaRedirect(request)
    return render(request, "payroll/disbursement/icici_config_form.html", {"form": form})
...
def generate_bank_file(request):
    """
    Generate a bank transfer file (Excel) for confirmed payslips.
    Includes employee bank details, amounts, and company source account.
    Optionally marks payslips as paid after download.
    """
    import io
    ...
    ids_str = request.GET.get("payslip_ids", "")
    mark_paid = request.GET.get("mark_paid", "") == "1"

    if ids_str:
        ids = [int(x) for x in ids_str.split(",") if x.strip().isdigit()]
        payslips = Payslip.objects.filter(id__in=ids, status="confirmed").select_related("employee_id")
    else:
        payslips = Payslip.objects.filter(status="confirmed").select_related("employee_id")

    if not payslips.exists():
        messages.warning(request, _("No confirmed payslips to export."))
        return redirect("salary-disbursement-view")

    # Get source account
    source_account = DisbursementBankAccount.objects.filter(is_active=True).first()
    ...
    ws.cell(row=row, column=1, value="SOURCE ACCOUNT (COMPANY)").font = section_font
    ws.cell(row=row, column=1).fill = section_fill
    row += 1

    if source_account:
        source_fields = [
            ("Bank Name", source_account.bank_name),
            ("Account Holder", source_account.account_holder_name),
            ("Account Number", source_account.account_number),
            ("Branch", source_account.branch or ""),
            ("IFSC / SWIFT", source_account.ifsc_code or ""),
            ("Company", str(source_account.company_id) if source_account.company_id else "All"),
        ]
        ...
        ws.cell(row=row, column=1, value="No source account configured")
        row += 1
    ...
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=8)
    ws.cell(row=row, column=1, value="PAYMENT DETAILS").font = section_font
    ws.cell(row=row, column=1).fill = section_fill
    row += 1

    summary_data = [
        ("Generated Date", date.today().strftime("%d-%b-%Y")),
        ("Total Payslips", str(payslips.count())),
        ("Total Amount", f"{sum(ps.net_pay or 0 for ps in payslips):,.2f}"),
        ("Pay Period", f"{payslips.first().start_date.strftime('%d-%b-%Y')} to {payslips.first().end_date.strftime('%d-%b-%Y')}"),
    ]
    ...
    headers = [
        "S.No", "Employee Name", "Employee ID", "Bank Name", "Account Number",
        "Branch", "IFSC / SWIFT Code", "Bank Code", "Net Pay Amount",
        "Pay Period", "Remarks",
    ]
    ...
    for idx, ps in enumerate(payslips, 1):
        ...
        emp_name = f"{ps.employee_id.employee_first_name} {ps.employee_id.employee_last_name}"
        badge = getattr(ps.employee_id, "badge_id", "") or ""
        period = f"{ps.start_date.strftime('%d-%b-%Y')} to {ps.end_date.strftime('%d-%b-%Y')}"
        ...
        row_data = [
            ...
            bank.branch if bank else "",
            getattr(bank, "any_other_code1", "") or "" if bank else "",
            getattr(bank, "any_other_code2", "") or "" if bank else "",
            ps.net_pay or 0,
            period,
            "NO BANK DETAILS" if not bank else "",
        ]
        ...
        total_amount += ps.net_pay or 0
        row += 1

    # Total row
    ws.cell(row=row, column=8, value="TOTAL").font = Font(bold=True)
    ws.cell(row=row, column=8).border = thin_border
    total_cell = ws.cell(row=row, column=9, value=total_amount)
    total_cell.font = Font(bold=True)
    total_cell.number_format = currency_fmt
    total_cell.border = thin_border

    # Column widths
    widths = [6, 25, 12, 18, 20, 18, 18, 15, 18, 25, 18]
    ...
    filename = f"Salary_Disbursement_{date.today().strftime('%Y%m%d')}.xlsx"
    response = HttpResponse(
        output.read(),
    ...
    if mark_paid:
        count = payslips.update(status="paid")
        # We can't add a Django message here since it's a file download,
        # but the status is updated in the DB

    return response


# ===========================Disbursement Bank Account CRUD================================


@login_required
@hx_request_required
@permission_required("payroll.add_disbursementbankaccount")
def create_or_update_disbursement_account(request, account_id=None):
    account = None
    if account_id:
        account = DisbursementBankAccount.objects.get(id=account_id)
    form = DisbursementBankAccountForm(instance=account)
    if request.method == "POST":
        form = DisbursementBankAccountForm(request.POST, instance=account)
        if form.is_valid():
            form.save()
            messages.success(request, _("Disbursement bank account saved successfully."))
            return HorillaRedirect(request)
    return render(
        request,
        "payroll/disbursement/disbursement_account_form.html",
        {"form": form},
    )
...
@login_required
@hx_request_required
@permission_required("payroll.delete_disbursementbankaccount")
def delete_disbursement_account(request, account_id):
    try:
        account = DisbursementBankAccount.objects.get(id=account_id)
        account.delete()
        messages.success(request, _("Disbursement bank account deleted."))
    except DisbursementBankAccount.DoesNotExist:
